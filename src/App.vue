...
const costs = computed(() =>
  additionalCost.value && !isNaN(parseInt(additionalCost.value))
    ? parseInt(additionalCost.value)
    : 0,
);
const paidDistance = computed(() =>
  distance.value && !isNaN(distance.value)
    ? (distance.value - freeDistance.value) * 2
    : 0,
);
const travelCost = computed(() =>
  (paidDistance.value * milageRate.value + costs.value).toFixed(2),
);
...
<template>
  <div>
    <div class="inputs">
      ...
      <h2 class="result" @click="copyToClipboard">
        Travel cost: <br />
        {{ travelCost }} zł
      </h2>
